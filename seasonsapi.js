const limit = 2;
let page = 1;
let episodios = []; // Variable para almacenar los episodios
...
function buscarTemporada() {
    ...
    fetch(urlApi)
        ...
        .then(response => response.json())
        .then(data => {
            episodios = data; // Guardamos todos los episodios
            page = 1; // Reiniciar la página al buscar una nueva temporada
            mostrarTemporada();
        })
        .catch(() => mostrarError());
}

// Generar opciones en el <select>
(() => {
...
function mostrarTemporada() {
    ...
    episodiosPagina.forEach(ep => {
        let episodioHTML = `
            <div class="episodio">
                ...
                <p><strong>Descripción:</strong> ${ep.summary ? ep.summary : "Sin descripción"}</p>
            </div>
            <hr>
        `;
    ...
    infoDiv.innerHTML += `
        <div class="paginacion">
            <button onclick="cambiarPagina(-1)" ${page === 1 ? "disabled" : ""}>Anterior</button>
            <button onclick="cambiarPagina(1)" ${end >= episodios.length ? "disabled" : ""}>Siguiente</button>
            <br>
...
// Mostrar error si la temporada no se encuentra
